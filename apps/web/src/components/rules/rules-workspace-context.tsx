...
import { getRouteApi, useBlocker, useNavigate, useRouterState } from "@tanstack/react-router";
...
export type RulesWorkspaceContextValue = ReturnType<typeof useRulesWorkspaceValue>;

const RulesWorkspaceContext = createContext<RulesWorkspaceContextValue | null>(null);

export function useRulesWorkspace(): RulesWorkspaceContextValue {
	const ctx = useContext(RulesWorkspaceContext);
	if (!ctx) {
		throw new Error("useRulesWorkspace must be used within RulesWorkspaceProvider");
	}
	return ctx;
}
...
export function RulesWorkspaceProvider({ children }: { children: ReactNode }) {
	...
	return <RulesWorkspaceContext.Provider value={value}>{children}</RulesWorkspaceContext.Provider>;
}
