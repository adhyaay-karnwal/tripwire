...
function AchievementBadge({ achievement }: { achievement: GitHubAchievement }) {
	...
	return (
		<div className="flex flex-col items-center gap-1 group" title={`${info.label}${info.tierLabel ? ` (${info.tierLabel})` : ""} — ${info.description}`}>
			...
			<span className="text-[10px] text-tw-text-tertiary group-hover:text-tw-text-secondary transition-colors text-center leading-tight">
				{info.label}
			</span>
		</div>
	);
...
function StatPill({ label, value, href }: { label: string; value: number; href?: string }) {
	const inner = (
		<span className="inline-flex items-center gap-1.5 text-[12px]">
			<span className="text-tw-text-primary font-medium tabular-nums">{value.toLocaleString()}</span>
...
export function CommunitySignals({
	...
	const hasBadges = graphql?.isGitHubStar || graphql?.isBountyHunter || graphql?.isDeveloperProgramMember || graphql?.isCampusExpert || graphql?.isSiteAdmin;
	const hasAchievements = achievements.length > 0;
	const hasStats = hasSponsors || hasSponsoring;

	if (!hasBadges && !hasAchievements && !hasOrgs && !hasStats && !hasSocials) return null;
			...
			<div className="rounded-[10px] bg-tw-inner p-3 flex flex-col gap-3">
				...
				{hasBadges && (
					<div className="flex items-center gap-1.5 flex-wrap">
						{graphql?.isGitHubStar && (
							<span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-amber-500/10 text-[11px] text-amber-300">
								GitHub Star
							</span>
						)}
						{graphql?.isSiteAdmin && (
							<span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-tw-accent/10 text-[11px] text-tw-accent">
								GitHub Staff
							</span>
						)}
						{graphql?.isBountyHunter && (
							<span className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-tw-inner text-[11px] text-tw-text-secondary">
								Bug Bounty Hunter
							</span>
						)}
				...
				{hasOrgs && (
					<div className="flex flex-col gap-1.5">
						<span className="text-[11px] text-tw-text-tertiary">Organizations</span>
						<div className="flex items-center gap-1.5 flex-wrap">
							{graphql!.organizations.map((org) => (
								<a
									...
									target="_blank"
									rel="noreferrer"
									className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-tw-card hover:opacity-80 transition-opacity"
								>
									<img src={org.avatarUrl} alt="" className="w-4 h-4 rounded-full" />
									<span className="text-[11px] text-tw-text-secondary">{org.login}</span>
								</a>
							))}
						</div>
					</div>
				...
				{hasSocials && (
					<div className="flex items-center gap-3 flex-wrap">
						{graphql!.socialAccounts.map((s) => (
							...
							>
								{s.provider === "TWITTER" ? "Twitter" : s.provider === "LINKEDIN" ? "LinkedIn" : s.url.replace(/^https?:\/\/(www\.)?/, "").replace(/\/$/, "")}
							</a>
						))}
					</div>
				)}
