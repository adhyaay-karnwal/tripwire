...
export const customRulesRouter = {
	...
	get: authedProcedure
		...
		.query(async ({ input, ctx }) => {
			...
			if (!rule) {
				throw new TRPCError({ code: "NOT_FOUND", message: "Custom rule not found" });
			}
	...
	create: authedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			if (!limits.canUseEnrichmentSignals && definitionReferencesEnrichment(input.definition)) {
				throw new TRPCError({
					code: "FORBIDDEN",
					message: "Enrichment signals (profile README, sponsors, badges, etc.) require a Pro plan.",
				});
			}
	...
	update: authedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			if (input.thresholdCount !== undefined) updateData.thresholdCount = input.thresholdCount;
			if (input.scopeOverride !== undefined) updateData.scopeOverride = input.scopeOverride;
		...
		.mutation(async ({ input, ctx }) => {
			...
			for (const username of uniqueUsernames) {
				...
				const avatarUrl = profile
					? ((profile as Record<string, unknown>).avatar_url as string | null) ?? null
					: null;
				...
				const result = evaluateCustomRule(input.definition as CustomRuleDefinition, signals);
