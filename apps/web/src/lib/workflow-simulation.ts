...
export type SingleWorkflowSimResult = {
	...
	outcomes: Array<{ nodeId: string; type: string; label: string; status: string; detail: string }>;
	actions: string[];
};
...
export async function fetchWorkflowRunContext(input: {
	...
	contentMeta: { title: string; number: number; url: string; state: string } | null;
}> {
	...
	if (token) {
		const [ghUser, mergedPrs, publicNonForkRepos, profileReadme, graphqlData, achievements, repoEvents] =
			await Promise.all([
...
function workflowDefinitionHasManualTrigger(def: WorkflowDefinition): boolean {
	...
	return nodes.some(
		(n) => n.type === "trigger" && (n.data?.trigger as string | undefined) === "manual",
	);
}
...
export function simulateWorkflowDefinition(
	...
	for (const t of triggers) {
		outcomes.push({
			...
			type: "trigger",
			label: (t.data as Record<string, unknown>)?.trigger as string ?? "trigger",
			status: "executed",
			detail: "Triggered",
		});
		...
		for (const edge of outs) {
			...
			if (targetNode.type === "rule") {
				...
				if (rule === "accountAge") {
					...
					pass = (ud.score as number ?? 0) >= ((params?.minScore as number) ?? 50);
			...
			outcomes.push({
				...
				label: (data.rule as string) ?? (data.action as string) ?? (data.gate as string) ?? (targetNode.type as string),
				status,
				detail,
			});
