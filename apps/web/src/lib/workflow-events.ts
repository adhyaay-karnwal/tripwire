...
export function extractWorkflowIdsFromMessages(messages: Array<{ role: string; parts?: Array<Record<string, unknown>> }>): string[] {
...
export function extractRuleIdsFromMessages(messages: Array<{ role: string; parts?: Array<Record<string, unknown>> }>): string[] {
  const ids = new Set<string>();
  for (const msg of messages) {
    if (msg.role !== "assistant" || !msg.parts) continue;
    for (const part of msg.parts) {
