...
export function buildChangeSummary(before: EditorSnapshot, after: EditorSnapshot): string {
  ...
  const parts: string[] = [];
  if (addedNodes) parts.push(`${addedNodes} node${addedNodes > 1 ? "s" : ""} added`);
  if (removedNodes) parts.push(`${removedNodes} node${removedNodes > 1 ? "s" : ""} removed`);
  if (editedNodes) parts.push(`${editedNodes} node${editedNodes > 1 ? "s" : ""} edited`);
  if (addedEdges) parts.push(`${addedEdges} edge${addedEdges > 1 ? "s" : ""} added`);
  if (removedEdges) parts.push(`${removedEdges} edge${removedEdges > 1 ? "s" : ""} removed`);
