...
export function attachDevRouterTiming(router: AnyRouter): void {
	...
	router.subscribe("onBeforeNavigate", (e: RouterEvents["onBeforeNavigate"]) => {
	...
	router.subscribe("onResolved", (e: RouterEvents["onResolved"]) => {
		if (navT0 === 0) return;
		if (!e.pathChanged && !e.hrefChanged) return;
		console.info(`[router] · resolve ${label} ${Math.round(performance.now() - navT0)}ms`);
	});

	router.subscribe("onRendered", () => {
		if (navT0 === 0) return;
		console.info(`[router] ✓ render ${label} ${Math.round(performance.now() - navT0)}ms`);
		reset();
	});
}
