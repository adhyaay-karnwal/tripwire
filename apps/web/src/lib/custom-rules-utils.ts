...
const ACTION_BADGE_MAP: Record<CustomRuleAction, { label: string; className: string }> = {
	...
	threshold: { label: "Threshold", className: "bg-tw-accent/15 text-tw-accent" },
};

export function getActionBadgeProps(action: CustomRuleAction): { label: string; className: string } {
