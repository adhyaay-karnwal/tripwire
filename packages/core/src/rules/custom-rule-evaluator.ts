import type { CustomRuleDefinition, CustomRuleNode, CustomRuleEdge } from "@tripwire/db";
...
function evaluateNumberCondition(actual: number, operator: string, threshold: number): boolean {
	switch (operator) {
...
function evaluateBooleanCondition(actual: boolean, operator: string, expected: boolean): boolean {
	switch (operator) {
...
function evaluateConditionNode(
	...
	if (!signalId || !operator) {
		return { passed: true, nearMiss: false, detail: "Incomplete condition (no signal or operator)" };
	}

	const actual = signals[signalId];
	if (actual === undefined || actual === null) {
		return { passed: true, nearMiss: false, detail: `Signal "${signalId}" not available` };
	}

	if (typeof actual === "number") {
		const threshold = Number(value);
		if (Number.isNaN(threshold)) {
			return { passed: true, nearMiss: false, detail: `Invalid threshold value: ${value}` };
		}
		...
		return { passed, nearMiss, detail: `${signalId} ${operator} ${threshold} (actual: ${actual})` };
	}

	if (typeof actual === "boolean") {
		const expected = value === true || value === "true";
		const passed = evaluateBooleanCondition(actual, operator, expected);
		return { passed, nearMiss: false, detail: `${signalId} ${operator} ${expected} (actual: ${actual})` };
	}
	...
	return { passed: true, nearMiss: false, detail: `Unsupported signal type for "${signalId}"` };
}
		...
		if (node.type === "condition") {
			...
			for (const edge of incoming) {
				...
				if (sourceOutcome !== undefined) {
					...
					if (sourceNode && (sourceNode.type === "condition" || sourceNode.type === "logic")) {
