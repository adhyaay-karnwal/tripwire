...
const ALLOWED_CUSTOM_RULE_NODE_TYPES = new Set(["condition", "logic", "transform"]);
...
const editCustomRule = defineTool({
	...
	handler: async ({ ruleId, operations }, ctx) => {
		...
		const current = rule.definition ?? { nodes: [], edges: [], outputNodeId: "" };
		const { state: next, errors, warnings } = applyWorkflowOperations(
